    ...
    module Service
      class Vm
        ...
        def create count=1
          ...
          RunLogger.level(1).color('cyan').info "create VM nodes"
          instances = Qingcloud::Instance.create(image_id,'small_a',count:count, login_mode: 'keypair')

          RunLogger.level(1).color('cyan').info "ask for IPs"
          ...
          (0...count).map do |i|
            RunLogger.level(1).color('cyan').info "bind ip: #{eips[i].address}"
